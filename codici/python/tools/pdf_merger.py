...
class PdfMergerApp(tk.Toplevel):
    def __init__(self, parent, data_path: Path):
        ...
        self.materie_per_anno = {
            "1° Anno": ["FISICA", "FONDAMENTI DI INFORMATICA", "COMPLEMENTI DI MATEMATICA", "ANALISI MATEMATICA", "ANALISI NUMERICA", "ELETTROTECNICA", "LINGUA INGLESE A"],
            "2° Anno": ["LINGUA INGLESE B", "MODELLISTICA E SIMULAZIONE", "FONDAMENTI DI AUTOMATICA", "CALCOLATORI ELETTRONICI E SISTEMI OPERATIVI", "SEGNALI E SISTEMI", "ALGORITMI E STRUTTURE DATI", "BASI DI DATI", "ELETTRONICA DEI SISTEMI DIGITALI"],
            "3° Anno": ["AUTOMAZIONE INDUSTRIALE", "RETI DI TELECOMUNICAZIONI", "RICERCA OPERATIVA", "INGEGNERIA DEL SOFTWARE", "MISURE MECCANICHE E TERMICHE", "SISTEMI PER LA GESTIONE DEI DATI", "LINUX E RETI", "SISTEMI ELETTRONICI E MISURE"]
        }
    ...
    def _create_widgets(self):
        ...
        self.materia_menu = tk.OptionMenu(main_frame, self.materia_selezionata, "")
    ...
    def crea_pdf_unito(self):
        ...
        try:
            ...
            merger = PdfMerger()
            for pdf in file_pdf:
                merger.append(str(percorso_cartella / pdf))

            with open(percorso_salvataggio, "wb") as file_unito:
                merger.write(file_unito)
            merger.close()

            if messagebox.askyesno("Operazione completata", f"File creato: {nome_file_output}\n\nVuoi eliminare i file PDF originali?", parent=self):
                for pdf in file_pdf:
